...
class DataTransform:
    ...
    def preprocess_data(self,data:pd.DataFrame)->pd.DataFrame:
        ...
        INF_SCALER_FILEPATH=os.path.join(self.TRANSFORMATION_CONFIG.artifacts_inferencing,self.TRANSFORMATION_CONFIG.scaler_filename)
        save_model(Path(SCALER_FILEPATH),scaler)
        save_model(Path(INF_SCALER_FILEPATH),scaler)
        ...
        for col in categorical_cols:
            ...
            INF_ENCODER_FILEPATH=os.path.join(self.TRANSFORMATION_CONFIG.artifacts_inferencing,f"{self.TRANSFORMATION_CONFIG.encoder_filename}_{col}.pkl")
            save_model(Path(ENCODER_FILEPATH),vectorizer)
            save_model(Path(INF_ENCODER_FILEPATH),vectorizer)
        
        logger.info('Standard scaler has been saved and numerical fields have been scaled')
        logger.info('Tfidf vectorizers have been saved but the data has not been transformed')
